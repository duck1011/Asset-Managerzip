    ...
    >
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5" data-testid="link-logo">
          ...
          <span className="text-2xl font-bold" style={{ letterSpacing: "-0.04em" }}>
        ...
        <div className="md:hidden flex items-center gap-2">
          <Link href="/dashboard" className="relative p-2 text-slate-300" data-testid="link-mobile-dashboard-icon">
          ...
          <button
            onClick={() => setLang(otherLang)}
            className="flex items-center gap-1 text-xs font-medium text-slate-300 hover:text-cyan-400 transition-colors border border-white/20 rounded px-2 py-1"
      ...
      {/* Mobile Nav Menu */}
      {isOpen && (
        <div className="md:hidden border-b border-white/10 bg-slate-900/95 backdrop-blur-xl" data-testid="mobile-menu">
          <div className="container mx-auto px-4 py-4 flex flex-col gap-4">
            {links.map((link) => (
              <Link
                ...
                className={`text-sm font-medium p-2 rounded transition-colors ${
                  location === link.href ? "text-cyan-400 bg-cyan-500/10" : "text-slate-300 hover:text-white hover:bg-white/5"
                }`}
            ...
            <Link
              ...
              className={`flex items-center gap-2 text-sm font-medium p-2 rounded transition-colors ${
                location === "/dashboard" ? "text-cyan-400 bg-cyan-500/10" : "text-slate-300 hover:text-white hover:bg-white/5"
              }`}
              onClick={() => setIsOpen(false)}
